...
export default function App() {
  ...
  type ExerciseStats = {
    exercise: string;
    maxWeight: number;    // PR: best single-set weight for this exercise
    totalReps: number;    // total reps logged across all sets for this exercise
    totalTonnage: number; // sum of weight*reps (rounded for display)
  };
  ...
  const highScores: ExerciseStats[] = useMemo(() => {
    ...
    for (const s of store.sets) {
      ...
      cur.totalReps += s.reps;                         // reps moved
      cur.totalTonnage += setVolume(s.weight, s.reps); // tonnage
      if (s.weight > cur.maxWeight) cur.maxWeight = s.weight; // PR
      byEx.set(s.exercise, cur);
    }
    const arr = Array.from(byEx.values());
    // Sort: PR first, then reps moved, then alpha
    arr.sort(
      (a, b) =>
  ...
  useEffect(() => {
    ...
    for (const { exercise, maxWeight } of highScores) {
      ...
      if (maxWeight > last) {
        nextUpdated[exercise] = Date.now(); // mark as NEW
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-neutral-100 p-6">
      {/* Keyframes for the floating NEW badge */}
      <style>{`
        @keyframes floatNew {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <section className="rounded-2xl border border-neutral-800 bg-neutral-950/70 p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-neutral-400">Highest weight • Reps moved • Tonnage</span>
          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {isNew && (
                              <span className="inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full
                                bg-amber-500/15 text-amber-300 ring-2 ring-amber-400/50
                                [animation:floatNew_1.5s_ease-in-out_infinite]">
                                NEW
                              </span>
                            )}
                          </div>
